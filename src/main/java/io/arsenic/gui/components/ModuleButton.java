package io.arsenic.gui.components;

import com.mojang.blaze3d.systems.RenderSystem;
import io.arsenic.Arsenic;
import io.arsenic.gui.Window;
import io.arsenic.gui.components.settings.*;
import io.arsenic.gui.components.settings.*;
...
public final class ModuleButton {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		Color toColor = module.isEnabled() ? Utils.getMainColor(255, Arsenic.INSTANCE.getModuleManager().getModulesInCategory(module.getCategory()).indexOf(module)) : Color.WHITE;
		...
		if (parent.moduleButtons.get(parent.moduleButtons.size() - 1) != this) {
			...
			context.fillGradient(parent.getX(), parent.getY() + offset, parent.getX() + 2, parent.getY() + parent.getHeight() + offset, Utils.getMainColor(255, Arsenic.INSTANCE.getModuleManager().getModulesInCategory(module.getCategory()).indexOf(module)).getRGB(), Utils.getMainColor(255, Arsenic.INSTANCE.getModuleManager().getModulesInCategory(module.getCategory()).indexOf(module) + 1).getRGB());
		} else {
